...
export default function ExploreCarousel({ scripts }: { scripts: ScriptItem[] }) {
      ...
      <div className="space-y-4">
        ...
        <div className="overflow-hidden mask-fade">
          <div className="flex gap-4 animate-scroll will-change-transform [backface-visibility:hidden] [transform:translateZ(0)]">
            {Array.from({ length: copies }).map((_, gIdx) => (
              <div key={`top-group-${gIdx}`} className="flex gap-4" aria-hidden={gIdx > 0}>
                {firstRowBase.map((script) => (
                  <div key={`top-${gIdx}-${script.id}`} className="shrink-0 w-[260px] sm:w-[300px]">
        ...
        <div className="overflow-hidden mask-fade">
          <div className="flex gap-4 animate-scroll-reverse will-change-transform [backface-visibility:hidden] [transform:translateZ(0)]">
            {Array.from({ length: copies }).map((_, gIdx) => (
              <div key={`bottom-group-${gIdx}`} className="flex gap-4" aria-hidden={gIdx > 0}>
                {secondRowBase.map((script: any, idx: number) => (
                  <div key={`bottom-${gIdx}-${script.id ?? idx}`} className="shrink-0 w-[260px] sm:w-[300px]">
                    {script.id === -1 ? <AddYourOwnCard /> : <CarouselCard script={script as ScriptItem} />}
                  </div>
                ))}
              </div>
            ))}
