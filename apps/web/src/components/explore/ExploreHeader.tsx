...
export default function ExploreHeader({ setOrderBy, orderBy }: ExploreHeaderProps) {
	...
	return (
		<>
			...
			<p
				...
				onClick={() => {
					const currentIndex = defaultOptions.indexOf(orderBy || defaultOptions[0]);
					const nextIndex = (currentIndex + 1) % defaultOptions.length;
					setOrderBy(defaultOptions[nextIndex]);
				}}
			>
				Explore the <span className="text-neutral-200 hover:text-white transition-colors duration-300 underline underline-offset-3 cursor-pointer">{getExploreText(orderBy || defaultOptions[1])}</span> apps at Dione.
			</p>
		</>
	);
}
