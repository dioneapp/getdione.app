...
export default function ExploreModal({
	...
	script: ScriptData;
	setSelectedScript: (script: ScriptData | null) => void;
}) {
	return (
		<motion.article
				...
				<div className="flex flex-col flex-1 h-full items-center mx-4 gap-4">
					<div className="flex flex-1 h-full max-h-full max-md:flex-col items-start justify-start gap-6 w-full">
						<div className="w-fit md:h-full flex md:flex-col items-start justify-start gap-2 max-md:border-0 md:border-r md:pr-6 md:border-white/10">
							...
							<div className="pr-0.5 flex flex-col gap-1 md:mt-4 max-md:mt-2 max-md:ml-4">
								...
								<span className="text-[10px] text-neutral-300 flex justify-left items-center text-balance">
									...
									<span className="text-neutral-200 mr-1">
										{script.updated_at
											? new Date(script.updated_at).toLocaleString(undefined, {
													dateStyle: "short",
													timeStyle: "short",
												})
											: ""}
								...
								<span className="text-[10px] text-neutral-300 flex justify-left items-center text-balance">
									...
									<span className="text-neutral-200 mr-1">{script.likes}</span>{" "}
									likes
								</span>
								<span className="text-[10px] text-neutral-300 flex justify-left items-center text-balance">
									<Tag className="w-3 h-3 mr-2" />
									<span className="text-neutral-200 mr-1 capitalize">
										{script.tags}
									</span>
								</span>
							</div>
						...
						<div className="flex flex-col gap-2 max-md:mt-4">
							...
							<div>
								<a
									href={script.author_url}
									target="_blank"
									rel="noopener noreferrer"
									className="text-neutral-300 text-xs"
								>
									Uploaded by{" "}
									<span className="text-[#BCB1E7] hover:underline underline-offset-2 cursor-pointer">
										{script.author}
									</span>
									{script.og_author && (
										<>
											{" "}
											& created by{" "}
											<span className="text-[#BCB1E7] hover:underline underline-offset-2">
												{script.og_author}
											</span>
											.
										</>
					...
					<div className="mt-auto h-fit w-fit flex justify-end items-end ml-auto">
						<div className="mt-auto gap-4 flex">
							<button
								...
								className="bg-white/10 hover:bg-white/20 text-neutral-300 border border-white/10 rounded-lg px-4 p-1 text-sm max-md:text-lg cursor-pointer duration-300 transition-colors"
							>
								Close
							</button>
							<button
								onClick={() =>
									(window.location.href = `dione://download=${script.id}`)
								}
								className="bg-white hover:bg-white/80 text-black font-semibold border border-white/10 rounded-lg px-4 p-1 text-sm max-md:text-lg cursor-pointer duration-300 transition-colors"
							>
								Install
							</button>
						</div>
