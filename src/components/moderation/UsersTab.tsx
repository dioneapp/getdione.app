...
export default function UsersTab() {
	...
	useEffect(() => {
		async function getSession() {
			const {
				data: { subscription },
			} = supabase.auth.onAuthStateChange((_, session) => setSession(session as Session));
	...
	const handleSaveUser = async (updatedUser: ExtendedUser) => {
		try {
			...
			if (error) throw error;
			setUsers(users.map((user) => user.id === updatedUser.id ? { ...user, ...updateData } : user));
			setEditingUser(null);
									...
									<div className="flex items-center justify-between">
										<div className="flex items-center gap-4">
											<div className="relative w-12 h-12 rounded-full overflow-hidden border-2 border-white/20 flex-shrink-0">
												{user?.avatar_url ? (
													<Image src={user.avatar_url} alt="Profile" fill className="object-cover" sizes="48px" />
												) : (
													<div className="w-full h-full bg-white/10 flex items-center justify-center">
														<span className="text-white/50 text-xl">{user?.username?.[0]?.toUpperCase() || "?"}</span>
													</div>
												)}
											</div>
											<div>
										...
										>
											<div className="p-4 space-y-4">
												...
												<div className="flex flex-col sm:flex-row justify-end gap-3 mt-4">
													{editingUser === user.id ? (
														<>
															<button onClick={cancelEditing} className="w-full sm:w-auto px-4 py-3 hover:text-white/70 rounded-lg text-white/80 transition-all duration-200 flex items-center justify-center gap-2 cursor-pointer">Cancel</button>
															<button onClick={() => editedUserData && handleSaveUser(editedUserData)} className="w-full sm:w-auto shrink-0 py-2 px-4 flex items-center justify-center gap-2 rounded-full bg-white font-semibold text-[#080808] cursor-pointer hover:bg-white/90 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-white/50 shadow-lg border border-black/10">
																<Check className="w-4 h-4" />
																Save Changes
															</button>
														</>
													) : profile?.moderator ? (
														<button onClick={() => startEditing(user)} className="px-4 py-2 bg-white/5 hover:bg-white/10 rounded-lg text-white/80 hover:text-white transition-all duration-200 flex items-center gap-2 cursor-pointer">
															<Pencil className="w-4 h-4" />
															Edit Details
														</button>
													) : (
														<div className="text-white/60 text-sm truncate">You need moderator privileges to edit user profiles</div>
													)}
												</div>
											</div>
										</motion.div>
