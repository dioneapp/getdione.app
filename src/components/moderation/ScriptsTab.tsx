...
export default function ScriptsTab() {
	...
	const [denyFeedbackScriptId, setDenyFeedbackScriptId] = useState<string | null>(null);
	...
	const submitFeedback = async (scriptId: string) => {
		if (!scriptId) return;
		const { error } = await supabase.from("scripts").update({ status: "denied", pending_review: false, review_feedback: feedbackText }).eq("id", scriptId);
		if (!error) setScripts(scripts.map(s => s.id === scriptId ? { ...s, status: "denied", pending_review: false, review_feedback: feedbackText } : s));
			...
			{loading ? (
				...
				<>
									...
									<div className="flex items-center justify-between">
										<div className="flex items-center gap-4">
											<div className="relative w-12 h-12 rounded-lg overflow-hidden border-2 border-white/20 flex-shrink-0">
												{script?.logo_url ? (
													<Image src={script.logo_url} alt={script.name} fill className="object-cover" sizes="48px" />
												) : (
													<div className="w-full h-full bg-white/10 flex items-center justify-center">
														<span className="text-white/50 text-xl">{script?.name?.[0]?.toUpperCase() || "?"}</span>
													</div>
												)}
											</div>
											<div>
										...
										>
											<div className="p-4 space-y-4">
												...
												<div className="flex flex-col sm:flex-row justify-end gap-3 mt-4">
													{editingScript === script.id ? (
														<>
															<button onClick={() => { setEditingScript(null); setEditedScriptData(null); }} className="w-full sm:w-auto px-4 py-3 rounded-lg text-white/80 font-semibold bg-white/10 hover:bg-white/20 active:bg-white/30 shadow transition-all flex items-center justify-center gap-2 cursor-pointer">cancel</button>
															<button onClick={() => editedScriptData && handleSaveScript(editedScriptData)} className="w-full sm:w-auto shrink-0 py-2.5 px-5 flex items-center justify-center gap-2 rounded-lg bg-white text-[#080808] font-bold cursor-pointer hover:bg-white/90 active:bg-white/80 transition-all shadow border border-black/10 focus:outline-none focus:ring-2 focus:ring-white/50">
														...
														<div className="w-full flex flex-col gap-2">
															...
															<div className="flex gap-2 justify-end">
																<button onClick={() => setDenyFeedbackScriptId(null)} className="px-5 py-2 rounded-lg bg-white/10 text-white/80 hover:bg-white/20 active:bg-white/30 shadow transition-all flex items-center gap-2 cursor-pointer">
																	<X className="w-4 h-4" />
																	Cancel
																</button>
																<button onClick={() => submitFeedback(script.id)} className="px-5 py-2 rounded-lg bg-red-500 text-white shadow border border-red-600 hover:bg-red-600 active:bg-red-700 transition-all cursor-pointer" disabled={!feedbackText.trim()}>Submit</button>
															</div>
														</div>
													) : (
														<>
															<button onClick={() => { setEditingScript(script.id); setEditedScriptData(script); }} className="px-4 py-2 rounded-lg bg-white/10 text-white/80 hover:bg-white/20 active:bg-white/30 transition-all flex items-center gap-2 cursor-pointer">
																<Pencil className="w-4 h-4" />
																Edit details
															</button>
															<button onClick={() => handleApprove(script.id)} className="px-5 py-2 rounded-lg bg-green-500 text-white shadow border border-green-600 hover:bg-green-600 active:bg-green-700 transition-all flex items-center gap-2 cursor-pointer">
																<Check className="w-4 h-4" />
																Approve
															</button>
															<button onClick={() => handleDeny(script.id)} className="px-5 py-2 rounded-lg bg-red-500 text-white shadow border border-red-600 hover:bg-red-600 active:bg-red-700 transition-all flex items-center gap-2 cursor-pointer">
																<X className="w-4 h-4" />
																Deny
															</button>
														</>
					...
					<div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
						...
						<div className="flex gap-2">
							<button onClick={() => setCurrentPage((p) => Math.max(1, p - 1))} disabled={currentPage === 1} className="p-2 bg-white/10 hover:bg-white/20 rounded text-white disabled:opacity-50 disabled:cursor-not-allowed transition-colors cursor-pointer" title="Previous Page">
								<ChevronLeft className="w-5 h-5" />
							</button>
							<button onClick={() => setCurrentPage((p) => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="p-2 bg-white/10 hover:bg-white/20 rounded text-white disabled:opacity-50 disabled:cursor-not-allowed transition-colors cursor-pointer" title="Next Page">
