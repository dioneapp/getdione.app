      ...
      >
        ...
        <div className="px-6 pt-6 pb-2">
          <div className="flex items-center justify-between">
            <h2 className="text-white text-2xl font-semibold" id="modal-title">{isEdit ? "Edit Script" : "Submit Script"}</h2>
            <button
              onClick={onClose}
          ...
          <p className="text-white/70 text-sm mt-1 mb-4">Share your script with the Dione community. Fill out the details below and submit for review.</p>
        </div>
          ...
          >
            ...
            <div className="flex gap-4 flex-col sm:flex-row">
              <div className="flex-1">
                <label htmlFor="script-name" className="block text-sm font-medium text-white mb-2">Name</label>
                <input
                  type="text"
                  id="script-name"
              ...
              <div className="flex-1">
                <label htmlFor="script-version" className="block text-sm font-medium text-white mb-2">Version</label>
                <input
                  type="text"
                  id="script-version"
                  placeholder="1.0.0"
            ...
            {/* description */}
            <div>
              <label htmlFor="script-description" className="block text-sm font-medium text-white mb-2">Description</label>
              <textarea
                id="script-description"
                placeholder="Short description"
                value={scriptForm.description}
                onChange={e => handleScriptField("description", e.target.value)}
            ...
            {/* script url */}
            <div>
              <label htmlFor="script-url" className="block text-sm font-medium text-white mb-2">Script URL</label>
              <input
                type="url"
                id="script-url"
                placeholder="https://github.com/user/repo/blob/branch/dione.json"
                value={scriptForm.script_url}
                onChange={e => handleScriptField("script_url", e.target.value)}
                className="w-full px-4 py-2 bg-white/10 border border-white/20 rounded-lg text-white placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-white/80 focus:border-white hover:border-white transition"
                required
                aria-required="true"
              />
            ...
            {/* logo url */}
            <div>
              <label htmlFor="logo-url" className="block text-sm font-medium text-white mb-2">Logo URL</label>
              <input
                type="url"
                id="logo-url"
            ...
            {/* banner url */}
            <div>
              <label htmlFor="banner-url" className="block text-sm font-medium text-white mb-2">Banner URL</label>
              <input
                type="url"
                id="banner-url"
                placeholder="https://i.imgur.com/banner.png"
                value={scriptForm.banner_url}
                onChange={e => handleScriptField("banner_url", e.target.value)}
                className="w-full px-4 py-2 bg-white/10 border border-white/20 rounded-lg text-white placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-white/80 focus:border-white hover:border-white transition"
                aria-required="true"
              />
            </div>
            ...
            {/* error */}
            {scriptError && (
              <div className="text-red-500 text-base font-semibold text-center bg-red-50/10 border border-red-500/30 rounded-lg py-2 px-3 mt-1">{scriptError}</div>
            )}
            {/* submit */}
            <button
              type="submit"
