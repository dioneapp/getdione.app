...
import AppCard from "../components/explore/card"
...
export default function Explore() {
    ...
    useEffect(() => {
        async function fetchData() {
            const {data, error} = await supabase.from('scripts').select('*').order('created_at', { ascending: false }).limit(24)
            if (error) {
                console.log(error)
            } 
    ...
    return (
        ...
        >
            <div
                className="bg-[#BCB1E7]/30 h-[70vh] w-[70vh] rounded-full blur-[150px]"
            >
            </div>
        </div>
        ...
        >
            ...
            <h1
                className="text-5xl font-semibold text-white tracking-tighter mt-6 text-balance text-center"
            >
                Explore
            </h1>
            <p
